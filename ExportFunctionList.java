...
public class ExportFunctionList extends GhidraScript implements Ingredient {
	...
	public void run() throws Exception {
		...
		while (iter.hasNext() && !monitor.isCancelled()) {
			...
			if (entry == null) {
				//pWriter.println("/* FUNCTION_NAME_ " + fName + " FUNCTION_ADDR_ " + "NO_ENTRY_POINT" + " */");
				println("WARNING: no entry point for " + fName);
			}
			else {
				pWriter.println("0x" + entry + " " + fName);
			}
		}
		pWriter.close();
	}
